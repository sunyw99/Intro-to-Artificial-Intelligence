...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            ...
            update[pos] = prob * self.beliefs[pos]
        self.beliefs = update
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for each in newPos.keys():
                update[each] = self.beliefs[pos] * newPos[each] + update[each]
        self.beliefs = update
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            distribution[p] += self.getObservationProb(observation, gameState.getPacmanPosition(),
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        #for _ in range(int(self.numParticles/len(particles))):
        #    for p in particles:
        #        self.particles.append(p)
        ...
        while num > length:
            self.particles += particles
            num -= length
        self.particles += particles[0 : num]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                acc *= self.getObservationProb(observation[i], gameState.getPacmanPosition(),
        ...
        for particle in self.particles:
            n = 1
            for i in range(self.numGhosts):
                n *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i],
                                             self.getJailPosition(i))
            dict[particle] += n

        for value in dict.values():
            if value != 0:
                self.particles = [dict.sample()
                                  for _ in range(self.numParticles)]
                return
