...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x, y, d = state
        if (x, y) in self.corners and sum(d) == 4:
            return True
        return False
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in self.corners:
                    ...
                    nextd[i] = 1
                child_state = (nextx, nexty, nextd)
                cost = 1
                successors.append([child_state, action, cost])
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        ...
        h = min(h, manhattan)
    if sum(d) == 2:
        h = h + problem.walls.height - 3
    elif sum(d) == 1:
        h = h + problem.walls.height - 3 + problem.walls.width - 3
    elif sum(d) == 0:
        h = h + 2 * (problem.walls.height - 3) + problem.walls.width - 3
    return h
...
def foodHeuristic(state, problem):
    ...
    # OWN IDEA (can achieve 7583 nodes expansion)
    ...
    if len(foodlist) > 0:
        ...
        for x in range(xmin, xmax):
            for y in range(ymin, ymax):
                if (x,y) in foodlist:
                    flag_food = True
                if (x,y) in problem.walls.asList() and flag_food:
                    wall_count += 1
                    flag_food = False
            if flag_food:
                h += wall_count
